...
describe User do
  before do
    ...
    @tweet = Tweet.create({ tweet: 'HI THERE MISTER HOW ARE YOU CLINTON', user_id: @user.id, search: 'election' })
  end
  ...
  describe '#tweet' do
    it "should return downcased tweet text" do
      expect(@tweet.tweet).to eq 'hi there mister how are you clinton'
    end
  end

  describe '.find_by_user_id' do
    it 'should return tweets based on user ID' do
    expect(Tweet.find_by_user_id(@user.id)).to eq  'hi there mister how are you clinton'
    end
  end

  describe '.find_by_search' do
    it 'should return all tweets found by query' do
      @tweets = Tweet.find_by_search('election', 'clinton')
      expect(@tweets).to eq 1
    end
  end
end
